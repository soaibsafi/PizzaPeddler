...
CREATE OR REPLACE FUNCTION hello(name VARCHAR) RETURNS VARCHAR AS $$ BEGIN IF name IS NULL THEN RETURN hello();
ELSEIF name = '' THEN RETURN hello('Anonymous');
...
CREATE OR REPLACE FUNCTION hello(name VARCHAR) RETURNS VARCHAR AS $$ BEGIN IF name IS NULL THEN name = 'World';
ELSEIF name = '' THEN name = 'Anonymous';
ELSEIF name = 'Bob' THEN name = 'Robert';
ELSEIF name = 'Bill' THEN name = 'William';
END IF;
...
CREATE OR REPLACE FUNCTION hello(name VARCHAR) RETURNS VARCHAR AS $$ BEGIN CASE
...
CREATE OR REPLACE FUNCTION rating(score INTEGER) RETURNS VARCHAR AS $$ BEGIN CASE
    score
    WHEN 1,
    2,
    3 THEN RETURN 'poor';
...
CREATE OR REPLACE FUNCTION rating_test() RETURNS SETOF VARCHAR AS $$ BEGIN FOR score IN 0..20 LOOP RETURN NEXT rating(score);
END LOOP;
RETURN;
END;
$$ LANGUAGE plpgsql;
...
DECLARE rnd INTEGER;
BEGIN LOOP rnd = ceil(random() * 100)::INTEGER;
RETURN NEXT rnd;
EXIT
WHEN rnd = 42;
END LOOP;
...
CREATE OR REPLACE FUNCTION random42() RETURNS SETOF INTEGER AS $$
DECLARE rnd INTEGER;
BEGIN LOOP rnd = (random() * 100)::INTEGER;
IF rnd = 0 THEN RAISE numeric_value_out_of_range;
END IF;
RETURN NEXT rnd;
EXIT
WHEN rnd = 42;
...
BEGIN LOOP rnd = (random() * 100)::INTEGER;
IF rnd = 0 THEN RAISE numeric_value_out_of_range;
ELSEIF rnd = 100 THEN RAISE error_in_assignment;
END IF;
RETURN NEXT rnd;
EXIT
WHEN rnd = 42;
